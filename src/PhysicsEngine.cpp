...
#include "color.h"
#include "Actor.h"
#include "MessagePhysicsCollision.h"
...
void PhysicsEngine::drawGeom(dGeomID geom)
{
	...
	{
	case dSphereClass:        drawGeomSphere(geom);         break;
	case dBoxClass:           drawGeomBox(geom);            break;
	case dCCylinderClass:     drawGeomCappedCylinder(geom); break;
	case dCylinderClass:      drawGeomCylinder(geom);       break;
	case dPlaneClass:         drawGeomPlane(geom);          break;
	case dRayClass:           drawGeomRay(geom);            break;
	case dTriMeshClass:       drawGeomTriMesh(geom);        break;
	case dGeomTransformClass: /* ignore */                  break;
	case dSimpleSpaceClass:   /* ignore */                  break;
	case dHashSpaceClass:     /* ignore */                  break;
...
void PhysicsEngine::drawGeomPlane(dGeomID geom)
{
	FAIL("stub");
}

void PhysicsEngine::drawGeomRay(dGeomID geom)
{
	FAIL("stub");
}
...
{
	// TODO: Get number of vertices from geom object
	const int numOfVertices = 63*63*2;
											   ...
											   dContact contact) const
{
	...
	if(a1)
	{
		passActorCollisionMessages(a1, o1, o2, contact);
	}

	Actor *a2 = reinterpret_cast<Actor*>(dGeomGetData(o2));
	if(a2)
	{
		passActorCollisionMessages(a2, o2, o1, contact);
	}
}

void PhysicsEngine::passActorCollisionMessages(Actor *actor,
											   dGeomID o1,
											   dGeomID o2,
											   dContact contact) const
{
	ASSERT(actor, "Parameter \"actor\" was null");
	actor->relayMessage(MessagePhysicsCollision(o1, o2, contact));
}
