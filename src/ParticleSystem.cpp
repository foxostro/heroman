...
void ParticleSystem::draw() const
{
		...
		j!=buckets.end(); ++j)
	{
		...
		GLsizei count = (GLsizei)bucket.elements.size();
		...
		glColorPointer(4, GL_FLOAT, 0, bucket.colors->getPointer());
		glTexCoordPointer(2, GL_FLOAT, 0, bucket.texcoords->getPointer());
		glVertexPointer(3, GL_FLOAT, 0, bucket.vertices->getPointer());
...
void ParticleSystem::updateParticleBatches(const mat4 &matrix)
{
		...
		j!=buckets.end(); ++j)
	{
		...
		color *colors = (color*)bucket.colors->lock();
		vec2 *texcoords = (vec2*)bucket.texcoords->lock();
		vec3 *vertices = (vec3*)bucket.vertices->lock();
		for(ParticleSet::const_iterator i=set.begin(); i!=set.end(); ++i)
		{
		...
		bucket.vertices->unlock();
		bucket.texcoords->unlock();
		bucket.colors->unlock();
	}
}
