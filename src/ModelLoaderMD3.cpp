...
void ModelLoaderMD3::Surface::create(FILE *stream)
{
	...
	{
		vertices = new Vertex[header.numVerts * header.numFrames];
		fseek(stream, surfaceStart + header.vertexOffset, SEEK_SET);
		fread(vertices, sizeof(Vertex), header.numVerts*header.numFrames, stream);
	}
}
                                               ...
                                               const size_t frame)
{
	...
	for(size_t i=0; i<count; ++i)
	{
	   r[i] = transformTriangle(tris[i], frame);
	}
    return r;
}
...
Mesh* ModelLoaderMD3::Surface::getObject(size_t frame) const
{
    ...
    const vector<Face> facesArray = getFaces(triangles, header.numTris, frame);
